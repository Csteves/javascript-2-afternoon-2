...
function divider(arr){
  ...
  let odds = arr.filter(i => i%2 !== 0);
  filterd.push(evens,odds);
  return filterd
}
...
function removeItem(myGroceryList,item){
  ...
  if(!myGroceryList || !item){
    ...
    myGroceryList.forEach(e =>{
      if(e !== item){
        listCopy.push(e);
      }
    })
    myGroceryList = listCopy;
...
let filterdEmployees = devMountainEmployees.filter(name => name !== cahlan);
devMountainEmployees = filterdEmployees;
...
var user2 = {
  name: 'Ty McGin',
  email: 'tylermcgin@gmail.com',
  password: 'iHateJavaScript',
  username: 'singleLoop'
}
var user3 = {
  name: ' McGinnis Tyler',
  email: 'mcginnisTyler@gmail.com',
  password: 'iLove/hateJavaScript',
  username: 'infinitewhileLoop'
}
